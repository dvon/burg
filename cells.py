...
F = 4  # How far apart must lines be?
...
# colors for highlighting columns (ethanschoonover.com/solarized)
...
def erase_borders(cell):
    h, w = cell.shape[:2]
    gray = cv2.cvtColor(cell, cv2.COLOR_BGR2GRAY)
    gray[(B * 2):-(B * 2), (B * 2):-(B * 2)] = 255

    # erase vertical borders
    ...
    for x in np.where(m_x < M * v_x)[1]:
        cv2.line(cell, (x, 0), (x, h), (255, 255, 255), 1)
    
    # erase horizontal borders
    ...
    for y in np.where(m_y < M * v_y)[0]:
        cv2.line(cell, (0, y), (w, y), (255, 255, 255), 1)
    
    # clean up leftover bits
    for x in np.where(m_x < M * v_x)[0]:
        k = np.ones((1, 3), dtype=np.uint8)
        cv2.morphologyEx(cell, cv2.MORPH_CLOSE, k, dst=cell)
    
    for y in np.where(m_y < M * v_y)[0]:
        k = np.ones((3, 1), dtype=np.uint8)
        cv2.morphologyEx(cell, cv2.MORPH_CLOSE, k, dst=cell)
...
def draw_cells(cropped, cells):
    ...
    for column in cells:
        for cell in column:
            x0, y0 = cell[0]
            x1, y1 = cell[1]
            erase_borders(cropped[y0:y1, x0:x1])
...
if __name__ == '__main__':
    ...
    cropped = crop(copy, m=30, f=6)
    draw_cells(cropped, cells(cropped))
    cv2.imwrite(n + '_cells.jpg', copy)
